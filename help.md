...
### chapter15: 位操作
- 二进制数、位和字节
> C语言中，可以单独操控变量中的位,有时候必须单独操控位，且很有用，C语言在提供高级语言的便利的同时
...
```
  ...
  2：原码，反码，补码
    正数的原码，反码，补码都相同，负数的原码就是对应正数原码的绝对值，反码时除去符号位全部取反
    补码时反码加1，即负数的补码就是把它正数的原码取反再加一就是了，或者是：求负数的补码，用它的正数，减一，再取反
      对于8位，带符号位置，原码和反码都不能表示-128，只能标识-127到+127
      其中-0=》10000000 +0=00000000
      因为两个不一样，所以才采用补码，那么补码的+0=00000000，-0=10000000  取反=》11111111  再加一=》00000000相同
      则此时多了一个1至127 着127个数字对应相反的-1至-127 共计254，加上0位255，还差一个（8位2禁止数据能标识256个数字）
      最后一个数字用补码：10000000标识，即-128
          也可以计算得出-128的补码标识方式，-128的补码就是其128的原码取反加一（或者原码减一再取反）
          正数对应的二进制：010000000 取反=》001111111 加一=》010000000 取8位 10000000
          正数对应的二进制：010000000 减一=》001111111 取反=》010000000 取8位 10000000
      二进制补码用1字节后7位标识数值，第一位标识符号位，对于正数只能表示0-127
      对于负数，用一个9位的二进制编码100000000减去一个负数的位组合，结果是该负数值的绝对值
      即256-该辅助二进制的绝对值，得到该负数二进制的绝对值，从而可以求出这个补码代表多少
          要得到一个二进制补码的相反数，最简单的方法就是反转每一位，然后加一（求反码+1）或者（-1求反码）
          例如求-3的补码，可以对其相反数3的补码取反+1，或者-1取反
            3的补码：00000011-》取反11111100-》加一 11111101，该数为-3的补码
                   00000011-》减一 00000010 取反-》11111101，同样得到-3的补码
      关于原反补，概括起来就三点
        1:正数的原反补相同，直接求二进制形式就行
        2:负数的补码用其绝对值（正数）的原码取反加1(或者减1取反)
        3:对于一个给定的负数的补码求负数二点值使用256-该绝对值，求出的值就是该负数的绝对值
        
      二进制的原码，反码都已两个零，+0，-0
        原码：+0：00000000   -0   10000000
        反码：+0：00000000   -0   11111111
        原码在符号位与数值位，补码存在符号位，补码正数存在数值位，补码负数则需要借助反码+1或者（-1反码）求得其绝对值
        反码是对所有位置取反们包括符号位
  3：二进制浮点数
    浮点数分两部分存储，1：二进制小数和二进制指数
    1：二进制小数
      10进制小数：0.521=5*10^-1+2*10^-2+1*10^-3=>5*1/10^1+2*1/10^2+1*1/10^3
      2进制小数0，101=1*2^-1+0*2^-2+1*2^-3=>1*1/2^1+0*1/2^2+1*/2^3
      许多分数如1/3无法用10进制标识，类似的许多分数也不能用2进制精确标识例如1/3和2/5
        实际上，任何进制只能精确的标识其进制分之一以及其幂的和
        二进制只能表示1/2及幂的和 10进制只能表示1/10及幂的和
    2：浮点数表示法
      二进制浮点数值：11.01=》1101*2^-2,可以将1101村存在二进制小数部门，将-2用存储在二进制指数部分
  4：其他进制（8，16都是2的幂，更接近计算机的二进制系统）
    1：8进制(octal)
      每个8进制位置对应三个二进制位置,0-7表示
    2：16进制(hex)        
      每个16进制位置对应四个二进制位置，0-15表示=》10-15用单个字母A-F表示，既可以大写也可以小写a-f
  5:操作位的工具
    1：6个按位运算符
      按位逻辑运算符
        四个按位逻辑运算符都用于整型数据，包括char,之所以叫按位，是因为操作时针对每一个位进行的，他们与逻辑运算符（&&，||，！）不同，后者是操作整个值
        1:按位取反~（二进制反码）
          00000010=2 使用~=>~(00000010)=255-2=253 (0-255一个循环)
        2:按位与&
          (10001000)&(01100110)=>00000000 两个位都为1时才为1（当两个位都为真时，才为真）
          &=:按位与与赋值运算符结合
          val&=0377=>val=val&0377
        3:按位或|
          (00100010)|(11001100)=11101110 两个位中有一个为1就为1(当两个位置有一个为真则为真)
          |=：按位或与赋值运算符结合
          val|=0377=》val=val|0377
        4:按位异或^
          (11001100)^(10011001)=01010101 两个位中只有一个为1时才为1（相同为0，不同为1）（两个只有一个为真时才为真）
          ^=:按位异或与赋值运算符结合
          val^=0377=>val=val^0377
      掩码：
        按为与运算符常用于掩码（mask）,所谓掩码指的是一些设置为开（1）或关（0）的位组合，为什么称其为掩码，看下面例子
        mask=00000010;flag=10011001
        flag=flag&mask; 
        flag现在的值是：00000000，把flag除第一位以外的值都置为0，
        整个过程叫做使用掩码，以为掩码中的0掩盖了flag中相应的位，把掩码中的0看作不透明，1看作透明，表达式flag&mask相当于用掩码覆盖在flags的位组上
        之后mask为1的位，flag上才可见，简写：&=
        flga&=mask;
          ch&=0xff=>ch&=0377=>ch&=(11111111);
          整个掩码保持ch中最后8位保持不变，其他位置都设为0，无论ch是8，16，24，32位，最终值都被修改为一个8位字节，该例子中，掩码宽度为8
      打开位：
        有时，需要打开一个值中的特定位，同时保持其他位不变
        flag|=mask=>10011011 //把1号位置设置为1，其他不变
      关闭位：
        和打开位类似，有时需要在不影响其他位置的情况下关闭指定位，
        flag=flag&~mask==>10111011&(~(00000010))=>10111011&(11111101)=>10111001
        简化：flag&=~mask  
      切换位：
        打开以关闭的位置，或者关闭以打开的位置，使用按位异或^运算符，0^b=b,相同为0，不同为1
        因此，如果使用异或组合一个值和一个掩码，将切换与掩码中1对应的位置的值，与0对应的位置的额值不变
        flag=flag^mask=>10011001^(00000010)=>10011011;//flag中与mask中1对应的位置都被切换了，与0对应的保持不变                 
      检查位的值
        有时候需要检查某个位的值是否打开（1）或关闭（0），不能直接拿值与掩码比较
        if(flag==mask),这样操作位即使打开，其他位置也不对，需要如下操作：
        if((flag&mask)==mask),用mask的掩码（最新的falg值）来与mask比较，以为经过mask掩码之后，最新的只只在掩码位置与mask不同，如果原本是打开的
        则其值就等于mask(比如原来是flag是：00000010，mask是：00000010，经过掩码之后flag还是：00000010，则它等价于掩码，所以1号位值是打开的（1）)
        为了避免信息漏过边界，掩码至少要与其覆盖的值宽度相同
      位移运算符：
        位移运算符分为向左和向右位移运算符
        左移运算符:<<
          将其左侧运算符对象每一位的值异动其右侧运算对象指定的位置，左侧移出的丢失，右侧用0补齐
          （10001010）<<2==>00101000,产生一个新的值，但是不改变其运算对象
          左移一位相当于*2
          <<= 用来位移之后赋值，例如 rest=2;rest<<=2//将rest的值修改为8
        右移运算符:>>
          将其左侧的运算对象每一位向右异动指定位数，移出右边末端的位丢弃，对于左端，对于无符号类型使用0填充
          对于有符号类型，其结果取决于机器，可用0（无符号右移）或者符号位填充（右移，或称有符号右移）
          10001010>>2
          有符号类型如下：
            00100010
            11100010
          无符号类型如下：
            00100010
          >>=:右移赋值
          int sweet=16;int noew;
          noew=sweet>>3;//noew=2,sweet的值仍然是16
          对于正数而言右移一位就是除以2，对于负数而言则要看系统是作为无符号唯一还是有符号位移
          具体参考上面右移策略
          思想类似于在十进制中移动小数点左右位置代表乘以10或除以10
        位移运算符还可以从较大的单元中提取一些位，例如使用unsigned long表示颜色的值（共四个字节），
        从右往左第一个字节存储红色，第二个字节存储绿色，第三个字节存储蓝色，随后你希望把每个颜色都存储到unsigned char类型变量中
        则可以使用以下语句：
          #define BYTE_MASK 0xff;//11111111
          unsigned long color=0x002a162f;//00101010 00010110 00101111
          unsigned char blue,green,red;
          //&BYTE_MASK将只保留最后的8位字节，以为掩码只有8位有效位，前面的的都将被置为0
          red=color&BYTE_MASK;//00101111
          green=(color>>8)&BYTE_MASK;//00010110   
          blue=(color>>16)&BYTE_MASK;//00101010
          以上，使用右移运算符将8位颜色移动至低阶字节，然后使用掩码技术将低阶字节赋值给变量
      编程实例：见bit_test.c  (位操作运算，位切换运算)
      ~切换所有位，^切换单个位（相同为0，不同为1，让位与1异或，就能反转该位的值），如果创建一个掩码，把后面的n位
      都设置为1，然后使用^异或组合整个掩码求值，就可切换一个数的后n位
    2：字段数据形式，用于访问int中的值    
      操控位的第二种方式是位字段，位字段是一个signed int 或者unsigned int类型变量中的一组相邻的位（C99,C11新增了_Bool类型的位字段）
      位字段通过一个结构来声明，该结构声明为每个字段提供标签，并确定该字段的宽度，例如如下声明一个4个1位的字段
        struct{
          unsigned int val1:1;
          unsigned int val2:1;
          unsigned int val3:1;
        ...
        根据该声明，结构prnt包含四个1位的字段，可以通过结构成员运算.给他们单独赋值
        prnt.val1=1;
        prnt.val2=0;  
        变量prnt被存储在int大小的内存单元，但是上面只使用了4位，也可以设置多位，如下：
        struct{
          unsigned int val1:1;
          unsigned int val2:2;
          unsigned int val3:8;
        ...
        prnt2.val1=1;
        prnt2.val2=3; 
        prnt2.val3=102;//确保所赋的值不超出字段可容纳的范围。
  ...
  通常把位字段作为一种更紧凑的存储数据的方式          
```
- 位字段和按位运算符
> 在同类编程问题中，位字段和按位运算符是两种可替换的方案，用哪种都可以,见源代码：dualview
- 对齐特性
> C11 的对齐特性比用位填充字节更自然，它们还代表了C在处理硬件相关问题上的能力，在这种上下文中，
> 对齐指的是如何安排对象在内存中的位置
```
_Alignof运算符给出一个类型的对齐要求，在关键字_Alignof后面的圆括号中写上类型名称即可：
  size_t d_align=_Alignof(float);
  假设d_align的值是4，意思是float类型对象的对齐要求是4，也就是说，4是存储该类型值相邻地址的字节数
  一般而言，对齐值是2的非负整数次幂，较大的对齐值被称为stricter或stronger,较小的对齐值被称为weaker
可以使用_Alignas说明符指定一个变量或类型的对齐值，但是不应该要求该值小于基本对齐值，例如float的对齐值是4
不要请求其对齐值为1,2,该说明符的作用是声明的一部分，说明符后面的圆括号包含对齐值或类型
...
- 关键概念
```
  1:C区别许多高级语言的特性之一就是访问整数中单独位的能力，该特性通常是硬件设备与操作系统交互的关键
  2:C有两种访问位的方法，一种是通过按位运算符，另一种是在结构中创建位字段
  3:C11新增了检查内存对齐要求的功能，且可以指定比基本对齐值更大的对齐值
  4:通常但不是绝对，在使用这些特性时，程序仅限于特定的硬件平台或操作系统，且设计为不可移植的
```
- 小结
```
  1:计算机硬件与二进制计数系统密不可分，一位二进制信通的0、1可以用于表示内存和寄存器中位的开闭状态，虽然C
  不允许以二进制形式书写数字，但是它识别与二进制相关的八进制和十六进制计数法，正如每个二进制数字表示1位一样，
  每个八进制数字代表3位，每个16进制数字代表4位
  2:c提供多种按位运算符，之所以称按位是因为他们单独操作一个值中的每一个位，~运算符将其运算对象每一位取反，
  按位与&运算符通过两个对象形成一个值，如果两个运算对象相同位置都为1，则位1，否则位0，
  按位或|运算符，当两个值中有一个为1则为1，否则为0，按位异或^运算符表示：对应的两位相同时为0，不同时为1，
  3:C还有左移<<和右移>>运算符，这两个运算符使位组合中的所有位都向左或向右异动指定数量的位，以形成一个新的值，
  空出的位置设置为0，如果是有符号类型的，右移的实现取决于计算机
  4:可以在结构中使用位字段操控一个值中单独位或多少位，
  5:可以使用_Alignas强制执行数据存储区上的对齐要求，_Alignof用于计算其标准的对齐值
```
...
```
  ...
  函数/宏：sqrt() atan() atan2() exit() atexit() assert() memcpy() memmove() va_start() va_arg() va_copy() va_end()
    在预处理之前没编译器会把多个物理行处理为一个逻辑行，1：查看程序
    预处理器根据预处理器指令，将符号缩写替换成其表示的内容，预处理器可以包含程序所需的其他文件，它并不知道c,基本上
    它的工作就是把一些文本转换成另外一些文本
    
    翻译程序第一步：
      在预处理之前，编译器必须对程序进行翻译处理，
        1：编译器将源代码中出现的字符映射到源字符集，处理多字节字符和三字节字符，字符扩展使c更加国际化
        2：编译器定位每个反斜杠后面跟着换行符的实例，把多个物理行转换为一个逻辑行 (注意，第2行要与第1行左对齐)
        3：编译器把文本分成预处理记号序列、空白序列、注释序列
            记号是由空格、制表符、换行符分隔的项
            编译器将用一个空格替换每一条注释，而且，实现可以用一个空格替换所有的空白字符序列（不包括换行符）
```
- 明示常量#define
> 预处理器从#开始运行，到后面第一个换行符为止，指令的长度仅限一行（逻辑行）
``` 
    --类对象宏  --类函数宏
    #define       PX      "THIS IS MY LIFE"
    预处理器指令     宏      替换体
    预处理器不做计算，不对表达式求值，它只进行替换
    #define FMT "X is %d"
    printf(FMT,x);
    或者： const char * fmt="X is %d.\n";
  何时使用字符常量
    1：绝大部分数字常量
    2：代表数组大小的数子也可以
    3：系统代码使用数字常量可移植
    4：现在支持得const关键字也支持该功能
      用const可以创建在程序运行时不能改变得变量，
      const可以具有文件作用域和块作用域
      宏常量可以用于指定标准数组得大小和const变量得初始值
      
```
- 在#define中使用参数
> 在#define中使用参数可以创建外形和作用与函数类似的类函数宏,
```
  #define MEAN(X,Y)  (((X+2)+(Y+2))/2)
  int x=MEAN(2,4);
  函数调用在程序运行时把参数得值传递给函数，宏调用在编译之前把参数记号传递给程序
...
```
- 用宏参数创建字符串：#运算符
> c允许在字符串中包含宏函数，在类宏函数的替换体中，#作为一个预处理运算符，可以把记号换成字符串
> 如果x是一个宏形参，#x就是转换为字符串的形参名称，这个过程称为字符串化，参考c_test.c
- 预处理器粘合剂：##运算符
> 与#类似，##可用于类函数宏的替换部分，还可以用于对象宏的替换部分，把两个记号组合成一个记号
> 注意：#运算符组合字符串，##运算符把记号组合为一个新的标识符
- 变参宏:...和__VA_ARGS__
> 通过把宏参数列表中最后的参数写成省略号...来实现可变参数这一功能，这样，预定义宏，__va_ARGS__可用在替换部分中
> 表明省略号代表什么,省略号只能代替最后的宏参数
- 宏和函数的选择
> 有些编程任务既可以使用函数又可以使用宏解决，如何选择可参考下面情况
> 1：使用宏比函数复杂些，稍有不慎会产生奇怪的作用
> 2：宏和函数的选择实际上是时间和空间的选择，宏生成内联代码，即在程序中生成语句
> 如果调用20次宏，则在程序中插入20行代码，如果调用函数20次，程序只有一份函数语句的副本
> 节省了空间，但是函数调用必须把程序的控制跳转至函数内，然后再返回主调程序，这显然比内联代码花费更多时间
> 3：宏的一个优点是不用担心变量类型，因为宏处理的是字符串而不是实际的值，因此只要是int或者float,都可以使用SQUARE(x)
> 4:C99提供了三种可替代的方案--内联函数
> 对于简单的函数，程序也通常使用宏：
```
  #define MAX(X,Y) ((X)>(Y)?(X):(Y))
  #define ABS(X) ((X)<0?(-X):(X))
  #define ISSIGN(X) ((X)=='+'||(X)=='-'?1:0) //如果x是一个代数符号字符，醉胡一个宏的值为1
  
  记住：宏名中不许有空格，但是替换字符串中可以有空格，用圆括号把参数都括起来，方便正确展开
  大写字母表示宏函数的名称，在嵌套循环中使用宏有助于提高效率
```
- 文件包含:#include指令
> 当预处理器发现#include指令时，会查看后面的文件名，并把文件的内容包含到当前文件中，即替换源文件的#include指令，
> 相当于将被包含文件的全部内容输入到源文件#include指令位置,有如下两种形式
```
  1:#include <stdio.h>  //尖括号告诉预处理器在标准系统目录中查找该文件
  2:#include "mystuff.h" //双引号告诉预处理器在当前目录中查找该文件(或文件名指定的路径)
    #include <stdio.h>//系统目录
    #include "hot.h" //查找当前工作目录
    #include "/bin/sh/dat.h"//查找文件指定目录
  问什么要包含头文件，一般，头文件中包含了很多函数的定义，以及许多类函数宏，c语言习惯用.h后缀表示文件头，这些文件包含了
  需要放在程序顶部的信息，头文件经常包含一些预处理器指令，函数的定义 
...
> 6：声明外部变量供其他文件共享
```
  int states=0;// 该变量具有文件作用域，在源代码文件
  然后，可以在与源代码文件相关联的头文件中进行引用式声明：
  extern int states;// 在头文件中
  这行代码会出现在包含了该头文件的文件中，这样使用该系列函数的文件都能使用这个变量，
  虽然源代码文件中包含该头文件后也包含了该声明，
  但是只要声明的类型一致，在一个文件中同时使用定义式声明和引用式声明没问题。
...
- 其他指令
``` 
  程序员通过修改#define的值即可生成可移植的代码，#undef指令取消之前的#define定义，#if,#ifdef
  #ifndef, #else ,#elif, #endif指令用于指定在什么情况下编写哪些代码，#line指令用于重置行和文件信息
  #error指令用于给出错误信息，#pragma指令用于向编译器发出指令
  ...
  1:#undef指令
    ...
    不确定之前是否已经用过，为安全起见，可以用#undef 指令取消该名字的定义。
    
    如果标识符是同一个文件中由前面的#define指令创建的宏名，且没有被#undef关闭，
    则该标识符是已定义的，如果是一个文件作用域的c变量，则对于预处理器而言是未定义的
      #define LIM 100  //已定义  直到文件尾或取消宏处
      #define GOOD  //已定义，空宏
    ...
    条件编译
      可以使用这些指令告诉编译器，根据编译时的条件执行或是忽略信息（代码块）
      #ifdef,#else,#endif
        #ifdef MAVIS
        #include "horse.h"//如果已经定义了MAVIS,则执行下面的指令
        #define STABLES 5
        #else
        #include "cow.h"//如果没有定义MAVIS,则执行下面的指令
        #define STABLE 15
        #endif
      指令可以嵌套，同时指令也可以标记C语句块
      int main(void){
        #ifdef MSG
        printf("cool.\n");
        #endif  
      ...
      #ifndef指令
        与#ifdef相似，也可以与#else,#endif一起使用，但是他们的逻辑相反，用于判断
        后面的标识符是未定义的，常用于定义之前未定义的常量
          #ifndef SIZE
          #define SIZE 100
          #endif
        还可以用于防止多次包含一个文件
          #ifndef THING_N_
          #define THING_N_
            如果该文件被重复包含多次，则第一次的时候，会定义THING
            下一次读取的时候就会跳过这段
        因为有的某些项（比如结构类型的声明）只能在一个文件中出现一次，这个#ifndef则帮助我们避免重复包含    
        
        如何确保待测试的标识符没有在别处定义。通常，实现的供应商使用这些方法解决这个问题：
        用文件名作为标识符、使用大写字母、用下划线字符代替文件名中的点字符、用下划线字符做前缀或后缀
        （可能使用两条下划线）
      #if,#elif指令
        后面跟整型常量表达式，如果非0，则表达式为真，可以使用关系运算符和逻辑运算符
          #if SYS==1
          #include "std.h"
          #elif sys==2
          #include "std2.h"
...
- 数学库
- tgmath.h库
> C99标准提供的tgmath.h头文件中定义了泛型类型宏，如果在math.h中为一个函数定义了3种类型（float、double和long double）的版本，
> 那么tgmath.h文件就创建一个泛型类型宏，与原来  double 版本的函数名同名
> 如果编译器支持复数运算，就会支持complex.h头文件，其中声明了与复数运算相关的函数。例如，声明有 csqrtf()、csqrt()和 csqrtl()，
> 这些函数分别返回 float complex、double complex和long double complex类型的复数平方根。如果提供这些支持，
> 那么tgmath.h中的sqrt()宏也能展开为相应的复数平方根函数，
...
- 通用工具库
```
  通用工具库包含各种函数，包括随机生数生成器，查找和排序树，转换函数和内存管理函数
  1：exit()
    在main返回系统时，将自动调用exit()函数
  ...
  2:atexit()
    atexit()函数通过退出时注册被调用的函数提供这种功能，接受一个函数作为参数指针,
    当调用exit()函数时会执行这些函数，ansi保证，在这个列表中，至少可以放32个函数，
    调用时，会以相反顺序执行这些函数，即最后添加的函数最先执行。
    atexit()注册的函数（如sign_off()和too_bad()）应该不带任何参数且返回
    类型为void。通常，这些函数会执行一些清理任务，例如更新监视程序的文件或重置环境变量
    注意，即使没有显式调用exit()，还是会调用sign_off()，因为main()结束时会隐式调用exit()。
  3:qsort()
    快速排序算法，
      void qsort(void *base,size_t nmember,size_t size,int(*compar)(const void*,const void*));
        ...
        计算出两个指针的值，然后把它们传递给比较函数。
             qsort()最后一个参数是一个指向函数的指针，该函数返回 int 类
             型的值且接受两个指向const  void的指针作为参数，这两个指针指向待比较项
      简而言之，为了让比较方法更具有通用性，参数使用了指向void的指针，在实际中，需要我们将其转换为具体的类型指针
```
- 断言库
> assert.h 头文件支持的断言库是一个用于辅助调试程序的小型库。它由assert()宏组成，接受一个整型表达式作为参数,
> 如果表达式求值为假（非零），assert()宏就在标准错误流（stderr）中写入一条错误信息，并调用abort()函数终止程序
> （abort()函数的原型在stdlib.h头文件中）。assert()宏是为了标识出程序中某些条件为真的关键位置，
> 如果其中的一个具体条件为假，就用 assert()语句终止程序。通常，assert()的参数是一个条件表达式或
逻辑表达式。如果 assert()中止了程序，它首先会显示失败的测试、包含测试的文件名和行号。
```
  assert()有几个好处：它不仅能自动标识文件和出问题的行
...
- _Static_assert
```
  assert()表达式是在运行时进行检查，C11新增了一个特性：_Static_assert声明，可以在编译时期
  检查assert()表达式，因此assert()可以导致正在运行的程序终止，_Static_assert()可以导致程序无法
  通过编译，它接受两个参数，第一个是整型常量表达式，第二个是字符串，如果第一个参数是0或_False,
  编译器会显示字符串，并且不再编译程序
    根据语法，_Static_assert()被视为声明，可以出现在函数中，或者出现在函数外部
    _Static_assert要求它的第1个参数是整型常量表达式，这保证了能在编译期求值（sizeof表达式被视为整型常量）
    不能用变量代替，同时当然也可以在main()函数中使用assert(CHAR_BIT  ==  16)，
    但这会在编译和运行程序后才生成一条错误信息，很没效率。
```
- string.h库的memcpy()和memmove()
```
  不能把一个数组赋值给另一个数组，所以要通过循环把一个数组的元素赋值给另一个，有一个例外的情况是：使用strcpy(),strncpy()
  他们复制字符数组，memcpy(),memmove()函数提供类似的方法处理任意类型的数组，原型如下：
     void *memcpy(void * restrict s1,const void * restrict s2,size_t n);
     void *memmove(void *s1,const void *s2,size_t n);
        关键字restrict假设两个内存区域没有重叠
        memcpy假设两个内存区域 没有重叠，memmove不做这样的假设
        n指代要拷贝的字节数，不同于元素，一般为n*sizeof(类型)
```
- 可变参数stdarg.h
```
  类似于变参宏，为函数提供了类似的功能，但是用法比较复杂，必须遵循以下步骤：
    1：提供一个使用省略号的原型//至少有一个形参和省略号，标准中用paramN表示第一个形参，代表省略号形参数量
    2：在函数定义中创建一个va_list类型的变量//用于存储形参列表中省略号部分的数据对象
    3：用宏把该变量初始化为一个参数列表//va_start(ap,lim),ap是存储对象,lim即paramN(形参数量)
    4：用宏访问参数列表//tic=va_arg(ap,double);检索特定类型的参数
    5：用宏完成清理工作//va_end(ap),调用va_end(ap)后，只有用va_start重新初始化ap后，才能使用变量ap。
    参考代码：varargs.c
    使用变参函数比使用变参宏更复杂，但是函数的应用范围更广
```
- 关键概念
```
  C标准不仅描述c语言，还描述了组成c语言的软件包，c预处理器和c标准库，通过预处理器可以控制编译过程，
  列出要替换掉额内容，指明要编译的代码行和影响编译器其他方面的行为，c库扩展了c语言的作用范围，为许多
  编程问题提供了现成的解决方案
```
- 本章小结
```
  C预处理器和C库是C语言的两个重要的附件，C预处理器遵循预处理器指令，在编译源代码之前调整源代码，C库提供许多
  有助于完善各种任务的函数，包括输入输出，文件处理，内存管理，排序与搜索，数学运算，字符串处理，参考完整的ANSI C库
```
...
```
- 研究数据表示
> 设计一种数据类型包括设计如何存储该数据类型和设计一系列的管理该数据的函数
- 从数组到链表
```
  参考代码films2.c
```
...
```
  类型特指两类信息，属性和操作
  假定要定义一个新的数据类型，首先需要提供存储数据的方法，例如设计一个结构
  其次是提供操控数据的方法，如添加修改删除等
  计算机领域的开发一种定义新类型的方法，步骤如下：
    1：提供类型属性和相关操作的抽象描述，即抽象数据类型ADT
    2：开发一个实现ADT的编程接口
      例如在C中提供结构定义以及操纵结构的函数原型
    3：编写代码实现接口
  以电影项目为例：
  1：建立抽象
    类型名：简单链表
    类型属性：可以存储一些列项
    类型操作：初始化链表为空，确定链表为空，已满，确定链表项数，末尾添加项，遍历链表，处理链表中的项，清空链表
  2：建立接口
      1:定义一种通用的Item类型，而不是int,struct film
      #define TSIZE 45
      struct film{
        char title[TSIZE];
        int rating;
      ...
      2:定义一种链表结构 
      typedef struct node{//每个链接叫做节点node,用Node作为结构的类型名
        Item item;
        struct node *next;
      } Node;
      typedef Node * List;//定义一个指向链表开始处的指针,定义链表
      ...
      注意要着重理解List movies声明创建了一个链表，而不是一个指向节点的指针或结构
  3：使用接口
      我们的目标是，使用这个接口编写程序，但是不必知道具体的实现细节（如不必知道函数的实现细节）
      在编写具体函数前，先编写程序的伪代码方案
        1：创建一个List类型的变量
        2：创建一个Item类型的变量
        3：初始化链表为空
        4：当链表未满且有输入时：把输入项读取到Item
        5：在链表末尾添加项
        6：访问链表中的每个项并显示他们
  4：实现接口 
      必须实现List接口，将代码实现为list.c，配合list.h,films3.c一起编译形成运行程序
      其中
        list.h: 定义数据结构和提供用户接口的原型
        list.c: 提供函数代码实现接口
        films3.c: 把链表接口应用于特定编程问题的源代码文件
```
- 队列ADT
```
  C语言中使用抽象数据类型方法编程包含以下三个步骤
  ...
  定义队列接口
    详见queue.h
    关于队列实现添加和删除问题，如果使用数组，则每次删除都会导致每一个数据向前移动一位，浪费计算时间
    当然也可以指定首位和末尾位置的指针，但是这样，每删除一个就会导致队列越来越小，因为首位置的指针不断的往后移
      解决这个问题的好办法就是使用环形数组队列：这以为这将数组的首尾相连，即数组的首元素紧跟在最后一个元素后面
      这样当数组到达末尾时，如果首元素空出，就可以把新添加的项存储到这些空出的元素中，当然要做一些标记，防止尾端超过首端
      当然除了环形数组，我们也可以使用链表，这样删除首项时不必移动其余元素，只需要重置头指针指向新的首元素即可
```
- 链表和数组
```
  许多编程问题，如创建一个简单链表和队列，都可以用链表（指的是动态分配结构的序列链）或数组来处理，每种方式都有优缺点
  具体如下：
    数据形式              优点                      缺点
    数组          c直接支持，提供随机访问          在编译时确定大小，插入删除很费时
    链表          运行时确定大小，快速插入和删除    不能随机访问，用户必须提供编程支持
  分析如下：
    在数组中插入元素：必须移动其他元素腾出空位插入新元素，距离数组头越近，被移动元素越多
    删除同理
    在链表中插入节点，只需要给两个指针赋值，一个赋值上一个节点的next节点为本节点，另一个将下一个节点赋值到本节点的next节点
    链表删除只需要设置前一个节点的next节点为本节点的next节点即可，并释放本届点内存，使用一个临时指针指向当前节点用于释放内存
    
    对于访问元素，数组可以通过下标随机访问，对于链表而言必须从头开始依次查询，顺序查找     
  二分查找：
    将带查找项目排序，，取中间值，如果大于中间值，则在后半部分二分查找，否则在前半部分二分查找，如果等于则停止
    确定中分点很简单，只需要将首尾索引相加并除以2，就得到中分点索引
  二叉查找树
    支持频繁的查找和频繁的插入和删除项  
```
- 二叉查找树
```
  二叉查找树是一种结合了二分查找策略的链接结构，二叉树的每个节点都包含了一个项和两个指向其他节点的指针（称为子节点）
  其顺序按照规定：左节点的项在父节点的项前面，右节点的项在父节点的项后面，这种关系存在于每一个有子节点的节点中，
  进一步而言，所有可以追溯其祖先回到一个父节点的左节点的项，都在该父节点项的前面，所有以一个父节点的右节点为祖先的项，都在该父节点后面
  最开始的节点称为root根部，如果二叉树是满的，则每一级的节点数是上一级节点数的2倍
  
  子数：二叉查找树的每个节点与其子节点构成了一个子树
  如果目标在根节点项的左边，则只需查找左子树，如果目标在根节点右边，则查找右子树
  每次比较就排除半个子树
  
  二叉树ADT
    一个非正式的数定义：
      类型名：二叉查找树
      类型属性：二叉树要么是空节点的集合（空树），要么是有一个根节点的节点结合
      ...
      类型操作：初始化树为空
              ...
              清空树
  二叉查找树接口
    原则上可以用多种方式来实现二叉查找树，甚至可以通过操作数组下标用数组来实现，但是实现二叉查找树最直接的办法就是
    通过指针动态分配链式节点
  ...
  使用二叉树
    详细代码见petclub.c   
  树的思想：
    二叉查找树也有一些缺陷，例如二叉查找树只有在满员（或平衡）时效率最高，假设用户输入的数据是按照顺序的，则该二叉查找树会变成类似的链表，因为添加的项总是在书的左边或右边
    查找这种树并不比查找链表快
      避免串状树的方法之一是在创建树时多加注意，如果树的一边或另一边不太平衡，就需要重新排列节点使之平衡，于此类似，可能在进行删除操作后需要重新排列树
      所以可以根据算法创建AVL:自平衡二叉查找树，所以创建一个平衡的树所花费的时间更多，但是这样的树可以确保最大化搜索效率。
      在AVL树中任何节点的两个子树的高度最大差别为1，所以它也被称为高度平衡树。增加和删除可能需要通过一次或多次树旋转来重新平衡这个树
  其他说明
    C已经涵盖了从简单的char类型变量到大型且复杂的结构在内的数据对象，面向对象语言进一步发展的对象的观点，
    例如，对象的性质不仅包括它存储的信息类型，而且还包括了对其进行的操作类型，我们介绍的ADT就遵循了这个模式，
    而且对象可以继承其他的对象的属性，OOP提供比C更高级的抽象，很适合编写大型程序    
```
- 关键概念
```
  一种数据类型通过一下几个点来表征：如何构建数据，如何存储数据，有哪些可能的操作
  抽象数据类型ADT以抽象的方式指定构成某种类型特征的属性和操作，把ADT翻译成两步：
  1:定义编程接口 2:实现接口
  在C中，可以用源代码文件提供与函数原型相应的函数定义来实现
```
- 小结
```
  链表、队列和二叉树是ADT在计算机程序设计中常用的示例，通常用动态内存分配和链式结构来实现他们，但有时使用数组会更好
  当使用一种特定类型（队列或树）进行编程时，需要根据类型接口来写程序
  
  定义一种数据类型包括确定如何存储数据，设计管理该数据的一些列函数
  ADT是抽象数据类型，是一种对类型属性集和可以对该类型进行操作的正式定义，用一般语言表示，且比包含实现细节
  函数中直接传递变量的优点意味着传递的是原始变量的副本，但是必须重新分配足够的内存空间
  传递变量地址的优点是可以修改原始数据，并且分配的内存会更少
  这种二叉查找树删除节点之后需要向右旋转节点已重新构成二叉树
